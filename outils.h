...
void on_key_press(GtkWidget *w, GdkEventKey *event, gpointer *data)
{
    fish *tmm;
    tmm = l;
    if(event->keyval == GDK_KEY_d){
        mouse_follow = 1;
        gtk_label_set_label(data[0],"MOUSE FOLLOW: <b>ON</b>");
    }
    else if(event->keyval == GDK_KEY_f){
        mouse_follow = 0;
        gtk_label_set_label(data[0],"MOUSE FOLLOW: <b>OFF</b>");
    }
    else if(l){
        while(tmm->suivant)
            tmm = tmm->suivant;
        if(event->keyval == GDK_KEY_s){
                tmm->control = 1;
                gtk_label_set_label(data[1],"FISH CONTROL: <b>ON</b>");
        }
        if(event->keyval == GDK_KEY_q){
                tmm->control = 0;
                gtk_label_set_label(data[1],"FISH CONTROL: <b>OFF</b>");
        }
        if(tmm->control)
        {
